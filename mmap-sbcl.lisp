...
(defun advance-stream (n stream)
  (let* ((position (mmap-stream-position stream))
    ...
    (setf (mmap-stream-position stream)
          new-position)
    (values (mmap-stream-sap stream) position)))

(declaim (inline read-n-bytes))
(defun read-n-bytes (n stream)
  (declare (optimize speed)
           (fixnum n))
  (multiple-value-bind (sap position) (advance-stream n stream)
   (funcall (ecase n
              (1 #'sb-sys:sap-ref-8)
              (2 #'sb-sys:sap-ref-16)
              (3 #'sap-ref-24)
...
(defun read-ascii-string-optimized (length string stream)
  ...
  (sb-sys:with-pinned-objects (string)
    (multiple-value-bind (sap position) (advance-stream length stream)
      (let ((mmap-sap (sb-sys:sap+ sap position))
